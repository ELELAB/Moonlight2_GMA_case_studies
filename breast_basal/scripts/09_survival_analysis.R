...
# Load clinical data
clinical_data <- get(load("breast_basal/data/BRCA_clinical_data.rda"))
...
save(cox_ph_test, file = "breast_basal/results/09_BRCA_basal_cox_ph.rda")

# Save results of Cox univariate regression analysis
save(cox_reg_uni_results, file = "breast_basal/results/09_BRCA_basal_cox_univariate.rda")

# Save results of Cox multivariate regression analysis
save(sig_genes_multivariate, file = "breast_basal/results/09_BRCA_basal_cox_multivariate.rda")

# Save overview of number of genes in each step of Cox regression
write_csv(num_genes, file = "breast_basal/results/09_BRCA_basal_cox_num_genes.csv")
...
map(km_sig_genes_multi_drivers, function(x) {
  ...
  if (!is.null(x) & length(x) != 0) {
    map(seq.int(length(x)), function(i) {
      ...
      pdf(str_c("breast_basal/results/09_survival_analysis_plots/09_BRCA_basal_km_survival_", names(x)[i], ".pdf"),
          width = 10, height = 10)
