...
data_DEA <- get(load("lung/data/LUAD_sig_DEA.rda"))
mutations_subset <- get(load("lung/data/LUAD_mutations_subset.rda"))
data_PRA <- get(load("lung/data/LUAD_dataPRA.rda"))

## Run the DMA function---------
data_DMA <- DMA(dataMAF = mutations_subset,
                 dataDEGs = DEA_lung,
                 dataPRA = PRA_lung,
...
save(data_DMA, file = "lung/data/LUAD_dataDMA.rda")
