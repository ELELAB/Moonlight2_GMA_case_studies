...
# Load FEA data
data_FEA <- get(load("lung/data/LUAD_dataFEA.rda"))

# Load DEA + methylation overview table
DEG_Methylation_Annotations <- get(load("lung/results/DEG_Methylation_Annotations.rda"))

# Load oncogenic mediators + number of methylated CpGs overview table
Oncogenic_mediators_methylation_summary <- get(load("lung/results/Oncogenic_mediators_methylation_summary.rda"))

# Load URA data
data_URA <- get(load("lung/data/LUAD_dataURA.rda"))
...
plotFEA(dataFEA = data_FEA_sig, 
        ...
        additionalFilename = "lung/results/05_LUAD_FEAplot.pdf",
        height = 6, 
        width = 10)
...
plotGMA(DEG_Methylation_Annotations = DEG_Methylation_Annotations, 
        ...
        type = "complete", 
        additionalFilename = "lung/results/05_LUAD_")
...
# Create Venn diagram 
venn.diagram(gene_lists, filename = "lung/results/05_LUAD_venn_gene_lists.png", 
