...
drug_gene_type_heatmap <- drug_gene_names_full_type %>% 
  ...
  scale_x_discrete(position = "top", guide = guide_axis(angle = 90)) +
  scale_y_discrete(position = "right") +
  scale_fill_manual(values = c("#6DCD59FF", "#FEC98DFF", "#21908CFF",
                               "#FCFDBFFF", "grey", "#FDE725FF")) +
  labs(x = "", y = "") +
  theme_classic() +
  theme(line = element_blank(),
        axis.text.x = element_text(size = 12,
...
save(drug_gene_interact, file = "lung/results/10_LUAD_drug_gene_interactions.rda")

# Save barplot showing number of drug-gene interactions of driver genes
ggsave(filename = "10_LUAD_drug_gene_interactions_num.pdf",
...
ggsave(filename = "10_LUAD_drug_gene_interactions_heatmap.pdf",
...
# Save heatmap showing drug-gene interactions of driver genes
ggsave(filename = "10_LUAD_drug_gene_interactions_type_heatmap.pdf",
