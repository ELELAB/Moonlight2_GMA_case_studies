...
# Load clinical data
clinical_data <- get(load("thyroid/data/THCA_clinical_data.rda"))

#clinical_data <- GDCquery_clinic(project = "TCGA-THCA", type = "clinical") %>% 
#  as_tibble()
...
save(cox_ph_test, file = "thyroid/results/09_THCA_cox_ph.rda")

# Save results of Cox univariate regression analysis
save(cox_reg_uni_results, file = "thyroid/results/09_THCA_cox_univariate.rda")

# Save results of Cox multivariate regression analysis
save(sig_genes_multivariate, file = "thyroid/results/09_THCA_cox_multivariate.rda")

# Save overview of number of genes in each step of Cox regression
write_csv(num_genes, file = "thyroid/results/09_THCA_cox_num_genes.csv")
...
map(km_sig_genes_multi_drivers, function(x) {
  ...
  if (!is.null(x) & length(x) != 0) {
    map(seq.int(length(x)), function(i) {
      ...
      pdf(str_c("thyroid/results/09_survival_analysis_plots/09_THCA_km_survival_", names(x)[i], ".pdf"),
          width = 10, height = 10)
