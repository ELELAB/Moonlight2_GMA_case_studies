...
drug_gene_type_heatmap <- drug_gene_names_full_type %>% 
  ...
  scale_x_discrete(position = "top", guide = guide_axis(angle = 90)) +
  scale_y_discrete(position = "right") +
  scale_fill_manual(values = c("#451077FF", "#000004FF", "#6DCD59FF",
                               "#721F81FF", "#21908CFF", "#F1605DFF", 
                               "grey")) +
  labs(x = "", y = "") +
  theme_classic() +
  theme(line = element_blank(),
...
save(drug_gene_interact, file = "thyroid/results/10_THCA_drug_gene_interactions.rda")

# Save barplot showing number of drug-gene interactions of driver genes
ggsave(filename = "10_THCA_drug_gene_interactions_num.pdf",
...
ggsave(filename = "10_THCA_drug_gene_interactions_heatmap.pdf",
...
# Save heatmap showing drug-gene interactions of driver genes
ggsave(filename = "10_THCA_drug_gene_interactions_type_heatmap.pdf",
